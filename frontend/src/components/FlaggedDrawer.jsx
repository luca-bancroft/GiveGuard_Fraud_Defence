export default function FlaggedDrawer({ submission, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="rounded-xl border border-yellow-500/30 p-6 w-full max-w-lg"
        style={{ backgroundColor: '#0f172a' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex justify-between items-start mb-4">
          <div>
            <p className="text-yellow-400 text-xs font-semibold mb-1">⚠ FLAGGED</p>
        ...
        <div className="flex items-center gap-3 mb-6 p-3 rounded-lg" style={{ backgroundColor: '#1e293b' }}>
          <span className="text-yellow-400 font-bold text-3xl">{submission.trust_score}</span>
          <div>
            <p className="text-white text-sm font-medium">Trust Score</p>
            <p className="text-gray-500 text-xs">out of 100 — review needed</p>
          </div>
        </div>
            ...
            <div className="space-y-2">
              {submission.signals.map((s, i) => (
                ...
                >
                  ...
                  <span className="text-yellow-400 text-xs font-bold ml-4 shrink-0">
                    +{s.risk_points} risk
                  </span>
                </div>
              ))}
        ...
        {!submission.signals?.length && (
          <div className="flex items-center gap-2 p-3 rounded-lg border border-yellow-500/20" style={{ backgroundColor: '#1a1400' }}>
            <span className="text-yellow-400 text-sm">⚠</span>
            <span className="text-yellow-300 text-sm">Flagged for manual review — no specific signals recorded</span>
          </div>
        )}
      </div>
    </div>

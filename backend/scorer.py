...
from irs_client import fetch_propublica_organization, looks_like_ein, normalize_ein
...
def score_submission(ein: str, org_name: str) -> dict[str, Any]:
	...
	osint_query = user_org_name or irs_org_name or (normalize_ein(cleaned_ein) if is_ein_input else cleaned_ein)
	osint_lookup = _fetch_osint_lookup(osint_query)

	osint_heading = str(osint_lookup.get("heading") or "").strip()
	if (
		not user_org_name
		and not irs_org_name
		and osint_heading
		and not osint_heading.isdigit()
		and osint_heading != normalize_ein(cleaned_ein)
	):
		lookup_name = osint_heading
